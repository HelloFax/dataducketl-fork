    ...
    def process!
      ...
      @tables.each do |table_or_class|
        ...
        begin
          ...
          redshift.query("UPDATE etl_event_log SET timestamp_end = CURRENT_TIMESTAMP,job_status = 'FAILED',error_code=#{ err.to_s }\n#{ err.backtrace.join("\n") }, runtime_in_s = EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - '#{ last_row[:timestamp_start] }')) WHERE id = #{ last_row[:id] }") # Added to gem per BI-560
